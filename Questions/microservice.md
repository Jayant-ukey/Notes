...
## Q1: Explain Trace ID in Microservices.

**Interview Answer:**

A **Trace ID** is a unique identifier generated for a request when it enters a microservice-based system. As the request travels through multiple microservices, the same Trace ID is propagated across all services.

Its main purpose is **distributed tracing**, which helps us track the complete journey of a request across multiple services.
...
When the request reaches the Order Service, a Trace ID is generated.

Example:

```
Trace ID = 9b7d4a21c5f648ab
```

The same Trace ID is passed to:
...
All logs generated by these services contain the same Trace ID.

Example logs:

```
[TraceId: 9b7d4a21c5f648ab] Order Created

[TraceId: 9b7d4a21c5f648ab] Payment Successful

[TraceId: 9b7d4a21c5f648ab] Inventory Updated

[TraceId: 9b7d4a21c5f648ab] Notification Sent
```

If something fails, we simply search using the Trace ID and can identify exactly where the request failed.

### How is it implemented?

In Spring Boot, Trace IDs are commonly handled using distributed tracing libraries such as:

* **Micrometer Tracing** (current standard in recent Spring Boot versions)
* **OpenTelemetry**
* Older projects may still use **Spring Cloud Sleuth**

The Trace ID is automatically propagated through HTTP headers to downstream services.

### Why is it important?

* End-to-end request tracking
* Easier debugging
* Faster production issue analysis
* Performance monitoring
* Better observability

---

## Q2: How do you manage transactions across Microservices?

**Interview Answer:**

Managing transactions across multiple microservices is different from handling transactions within a single database.

A traditional ACID transaction cannot usually span multiple microservices because each service owns its own database.

Instead, we use **eventual consistency** and distributed transaction patterns.

The most common approach is the **Saga Pattern**.
...
If every step succeeds, the overall business transaction completes successfully.

### What if Payment succeeds but Inventory fails?

Instead of rolling back everything using a global database transaction, Saga performs a **compensating transaction**.
...
Each service reverses only its own work.
...
* Harder to understand and debug as the number of services grows
...
If one step fails, the orchestrator invokes compensating actions.
...
* The orchestrator becomes a central component that must be highly available

---

### Why not use Two-Phase Commit (2PC)?

Although **Two-Phase Commit** provides strong consistency, it is generally avoided in microservices because:

* It can block resources while waiting for all participants.
* It reduces scalability and availability.
* It introduces a central coordinator and becomes a bottleneck.
* It does not align well with independently deployable services.

For these reasons, most modern microservice architectures prefer the Saga pattern with eventual consistency.

---

### What technologies have you used?

A strong practical answer might be:

> "In our Spring Boot microservices, each service manages its own database transaction using `@Transactional`. For cross-service communication, we publish events through a message broker such as Apache Kafka. We propagate Trace IDs using Micrometer Tracing/OpenTelemetry for end-to-end observability. For business workflows involving multiple services, we follow the Saga pattern with compensating transactions instead of distributed database transactions."

This answer demonstrates that you understand both the concepts and how they are applied in real-world Java microservice applications.
